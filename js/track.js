import { SHIPMENTS_URL, STATUS_ORDER, STATUS_META } from "../data/trackingData.js";
...
export class TrackUI {
  ...
  async init() {
    ...
    if (!this.resultsEl || !this.searchBtn || !this.codeInput || !this.phoneInput || !this.tabCode) {
  ...
  onSearch() {
    ...
    const query = byCode ? this.codeInput.value : this.phoneInput.value;
    ...
    const steps = STATUS_ORDER.map((label, i) => {
      ...
      return `
        <li class="step ${done ? "done" : ""} ${current ? "current" : ""}">
          <span class="step-icon material-symbols-outlined" style="color:${done ? meta.color : "#475569"}">
            ${meta.icon}
          </span>
